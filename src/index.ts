    ...
    fs.readFile(BACKUP_NAME)
        .then((buffer: Buffer) => {
            ...
            try {
                ...
                if (!validateCanvasJSON(json)) throw new Error('Invalid format in backup');
            } catch (err) {
                console.error(err);
                console.error(
        ...
        .catch(async (err) => {
            ...
            if (writeSuccessful) console.log('Created backup file');
        })
        .finally(() => {
...
function verifyTileData(data: any): boolean {
    ...
    return (
        ...
        data[LOCATION_X] >= 0 &&
        data[LOCATION_X] < cols &&
        typeof data[LOCATION_Y] === 'number' &&
        data[LOCATION_Y] >= 0 &&
        data[LOCATION_Y] < rows &&
        typeof data[COLOR] === 'number' &&
        data[COLOR] >= 0 &&
        data[COLOR] < NUM_OF_COLORS
    );
}
...
function isValidTileLocation(location: any): boolean {
    return typeof location === 'string' && /^\d+,\d+$/.test(location);
}

function isValidTileColor(color: any): boolean {
    return typeof color === 'number' && color >= 0 && color < NUM_OF_COLORS;
}
